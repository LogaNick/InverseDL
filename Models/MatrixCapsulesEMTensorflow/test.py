...
from Models.MatrixCapsulesEMTensorflow.config import cfg, get_coord_add, get_dataset_size_test, get_num_classes, get_create_inputs
import time
import numpy as np
import sys
import os
...
def main(args):
    ...
    # !!! is_train=False, epochs=1
    create_inputs = get_create_inputs(dataset_name, is_train=False, epochs=1)
    ...
    with tf.Graph().as_default(), tf.device('/cpu:0'):
        ...
        # read snapshot
        # !!! You have to put logdir in repo's root directory
        ...
        if not os.path.exists(cfg.test_logdir + '/caps/{}/test_log/'.format(dataset_name)):
            os.makedirs(cfg.test_logdir + '/caps/{}/test_log/'.format(dataset_name))
        summary_writer = tf.summary.FileWriter(
            cfg.test_logdir + '/caps/{}/test_log/'.format(dataset_name), graph=sess.graph)  # graph = sess.graph, huge!

        """Main loop."""
        # m_min = 0.2
        # m_max = 0.9
        # m = m_min
        m = 0.9 # !!! Use final margin directly
        for step in range(cfg.epoch * num_batches_per_epoch + 1):
            ...
            try:
                ...
                # We are doing testing, so remove train_op.
